...
import io.contentos.sdk.crypto.Hash;
import io.contentos.sdk.encoding.Zlib;
import io.contentos.sdk.prototype.Transaction.operation;
import io.contentos.sdk.prototype.Type;
import io.contentos.sdk.prototype.Operation.*;
...
public class Operation {
    ...
    public interface OperationProcessor<Result> {
        ...
        /**
         * Process an operation of vest delegation (lending)
         * @param from          name of the account who lends vests
         * @param to            name of the account who receives vests
         * @param amount        amount of vests
         * @param expiration    time in blocks after which the lent vests can be claimed
         * @return processing result
         */
        Result delegateVest(String from, String to, long amount, long expiration);

        /**
         * Process an operation of vest delegation claim
         * @param account       name of the account who claims the lent vests
         * @param orderId       the order-id of lent vests
         * @return processing result
         */
        Result unDelegateVest(String account, long orderId);
    }
    ...
    public static abstract class BaseResultFilter<SrcType, Upstream extends OperationProcessor<SrcType>, DstType> implements OperationProcessor<DstType> {
        ...
        public DstType delegateVest(String from, String to, long amount, long expiration) {
            return filterResult(upstreamFactory.newInstance().delegateVest(from, to, amount, expiration));
        }

        public DstType unDelegateVest(String account, long orderId) {
            return filterResult(upstreamFactory.newInstance().unDelegateVest(account, orderId));
        }
    }
    ...
    public static class OperationCreator implements OperationProcessor<operation> {
        ...
        public operation delegateVest(String from, String to, long amount, long expiration) {
            return operation.newBuilder().setOp23(
                    delegate_vest_operation.newBuilder()
                            .setFrom(Type.account_name.newBuilder().setValue(from))
                            .setTo(Type.account_name.newBuilder().setValue(to))
                            .setAmount(Type.vest.newBuilder().setValue(amount))
        ...
        public operation unDelegateVest(String account, long orderId) {
            return operation.newBuilder().setOp24(
                    un_delegate_vest_operation.newBuilder()
                            .setAccount(Type.account_name.newBuilder().setValue(account))
                            .setOrderId(orderId)
            ).build();
        }
