package io.contentos.sdk.rpc;

import com.google.protobuf.ByteString;

import java.util.zip.CRC32;

import io.contentos.sdk.prototype.MultiId;
import io.contentos.sdk.prototype.Transaction.signed_transaction;
import io.contentos.sdk.prototype.Type;
import io.contentos.sdk.grpc.Grpc.*;
...
public class RpcClient extends Operation.BaseResultFilter<Transaction, Transaction, BroadcastTrxResponse> {
    ...
    /**
     * Get voters of given block producer
     * @param producer    account name of the producer
     * @param pageSize   maximum voters in a page
     * @return voter list in ascending order of voter account names.
     */
    public RpcResultPages<GetBlockProducerVoterListResponse, Type.account_name, Type.account_name> getBlockProducerVoterList(final String producer, int pageSize) {
        return new RpcResultPages<GetBlockProducerVoterListResponse, Type.account_name, Type.account_name>(
                minAccountName,
                maxAccountName,
                pageSize)
        {
            @Override
            protected GetBlockProducerVoterListResponse request(Type.account_name start, Type.account_name end, int count, Type.account_name lastVoter) {
                GetBlockProducerVoterListRequest.Builder b = GetBlockProducerVoterListRequest.newBuilder();
                b.setBlockProducer(accountName(producer)).setLimit(count);
                if (lastVoter != null) {
                    b.setLastVoter(lastVoter);
                }
                return service.getBlockProducerVoterList(b.build());
            }

            @Override
            protected Type.account_name getLastItem(GetBlockProducerVoterListResponse resp) {
                return isEmptyResponse(resp)? null : resp.getVoterList().get(resp.getVoterList().size() - 1).getAccountName();
            }

            @Override
            protected Type.account_name keyOfValue(Type.account_name value) {
                return value;
            }

            @Override
            public boolean isEmptyResponse(GetBlockProducerVoterListResponse resp) {
                return resp == null || resp.getVoterCount() == 0;
            }
        };
    }

    /**
     * Get vest delegation orders of given account.
     * @param name    account name
     * @param isFrom  true: delegations from the account; false: delegations to the account.
     * @param pageSize   maximum orders in a page
     * @return vest delegation order list.
     */
    public RpcResultPages<GetVestDelegationOrderListResponse, Object, Long> getVestDelegationOrderList(final String name, final boolean isFrom, int pageSize) {
        return new RpcResultPages<GetVestDelegationOrderListResponse, Object, Long>(
                null,
                null,
                pageSize)
        {
            @Override
            protected GetVestDelegationOrderListResponse request(Object start, Object end, int count, Long lastOrderId) {
                GetVestDelegationOrderListRequest.Builder b = GetVestDelegationOrderListRequest.newBuilder();
                b.setAccount(accountName(name)).setIsFrom(isFrom).setLimit(pageSize);
                if (lastOrderId != null) {
                    b.setLastOrderId(lastOrderId);
                }
                return service.getVestDelegationOrderList(b.build());
            }

            @Override
            protected Long getLastItem(GetVestDelegationOrderListResponse resp) {
                return isEmptyResponse(resp)? null : resp.getOrdersList().get(resp.getOrdersList().size() - 1).getId();
            }

            @Override
            protected Object keyOfValue(Long value) {
                return null;
            }

            @Override
            public boolean isEmptyResponse(GetVestDelegationOrderListResponse resp) {
                return resp == null || resp.getOrdersCount() == 0;
            }
        };
    }
    ...
    private static final Type.time_point_sec minTimeStamp = timeStamp(0);
    private static final Type.time_point_sec maxTimeStamp = timeStamp(Integer.MAX_VALUE);

    private static Type.account_name accountName(String name) {
        return Type.account_name.newBuilder().setValue(name).build();
    }

    private static final Type.account_name minAccountName = accountName("");
    private static final Type.account_name maxAccountName = accountName("zzzzzzzzzzzzzzzzz");
}
